    ...
    public static function run(Request $request = null) {
        ...
        try {
            ...
            if (is_file(APP_PATH . $route->getRoute()['app'] . '/route.php')) {
                ...
                Response::sendError(FireCode::ERR_ROUTE_APP_FILE_INEXISTED, FIRE_MSG[FireCode::ERR_ROUTE_APP_FILE_INEXISTED]);
            }
//            Logger::debug("加载路由文件：", [Config::get('route')]);
            $config = Config::get();
            // 路由检查，顺带做参数校验
            $routes = fire::checkRoute($route, $config['route']);
            ...
            Response::sendException(FireCode::ERR_APP_RUN, FIRE_MSG[FireCode::ERR_APP_RUN], $e);
        }
        Logger::info("FIRE_END");
            ...
            foreach ($methods as $method) {
                if ($method->getName() == $route['action']) {
                    ...
                    if ($method->isPrivate() || $method->isConstructor() || $method->isStatic() || $method->isDestructor()) {
                        Response::sendError(FireCode::ERR_REQUEST_ACTION_TYPE, FIRE_MSG[FireCode::ERR_REQUEST_ACTION_TYPE]);
                    }
...
//                    return $method->invokeArgs($instance, $route['inputs']);
            ...
            Response::sendError(FireCode::ERR_ROUTE_ACTION, FIRE_MSG[FireCode::ERR_ROUTE_ACTION]);
        } catch (\ReflectionException $e) {
            Response::sendException(FireCode::ERR_ROUTE_REFLECTION_FAILED, FIRE_MSG[FireCode::ERR_ROUTE_REFLECTION_FAILED], $e);
        }
        ...
        if ($reflect->getNumberOfParameters() > 0) {
            ...
            foreach ($params as $param) {
                ...
                if ($class) {
                    ...
                    Response::sendException(FireCode::ERR_REQUEST_PARAM_INEXIST, APP_MSG[FireCode::ERR_REQUEST_PARAM_INEXIST],
