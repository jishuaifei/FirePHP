...
class Logger {
    ...
    static private function make($logType, $textType, $info = []) {
        $lineBreak = "\n";
        $log       = '';
        $fireLog   = Config::get('fire_log');
        $appLog    = Config::get('app_log');

        $logText = empty($fireLog[self::$LogLevel[$logType]][$textType]) ? $appLog[self::$LogLevel[$logType]][$textType] : $fireLog[self::$LogLevel[$logType]][$textType];

        if ($textType == "FIRE_END") {
            $lineBreak = "\n\n";
        }
        if (empty($logText)) {
