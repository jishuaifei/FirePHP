        ...
        if ($type == Validate::PARAM_STRING) {
            if ($pattern != '' && $pattern != null) {
                if (!preg_match($pattern, $value)) {
                    return FireCode::ERR_REQUEST_PARAM_STRING;
                }
            } else {
                if ($value == '' || $value == null) {
                    return FireCode::ERR_REQUEST_PARAM_STRING;
            ...
            if (!preg_match($pattern, $value)) {
                return FireCode::ERR_REQUEST_PARAM_TIME;
            }

        } else if ($type == Validate::PARAM_PHONE) {
            if (!preg_match("/^1[34578]{1}\d{9}$/", $value)) {
                return FireCode::ERR_REQUEST_PARAM_PHONE;
            }
        } else if ($type == Validate::PARAM_EMAIL) {
            if (filter_var($value, FILTER_VALIDATE_EMAIL) == false) {
                return FireCode::ERR_REQUEST_PARAM_EMAIL;
            }
        } else if ($type == Validate::PARAM_NUMBER) {
            if ($pattern != '' && $pattern != null) {
                if (!preg_match($pattern, $value)) {
                    return FireCode::ERR_REQUEST_PARAM_NUMBER;
                }
            }
            ...
            if (!is_array($pattern)) {
                return FireCode::ERR_REQUEST_PARAM;
            }
            if (!in_array($value, $pattern)) {
                return FireCode::ERR_REQUEST_PARAM_OPTION;
            }
        } else if ($type == Validate::PARAM_MD532) {
            if (!preg_match("/^[a-z0-9]{32}$/", $value)) {
                return FireCode::ERR_REQUEST_PARAM_MD532;
            }
        } else if ($type == Validate::PARAM_JSON) {
            json_decode($value);
            if (!(json_last_error() == JSON_ERROR_NONE)) {
                return FireCode::ERR_REQUEST_PARAM_JSON;
            }
        } else if ($type == Validate::PARAM_IP) {
            if (filter_var($value, FILTER_VALIDATE_IP) == false) {
                return FireCode::ERR_REQUEST_PARAM_IP;
            }
        } else if ($type == Validate::PARAM_URL) {
            if (filter_var($value, FILTER_VALIDATE_URL) == false) {
                return FireCode::ERR_REQUEST_PARAM_URL;
            }
        } else {
            if ($pattern != '' && $pattern != null) {
                if (!preg_match($pattern, $value)) {
